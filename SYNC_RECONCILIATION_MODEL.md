...
- **`attribution_diff` case** = a `sync_held_cases` row, `kind='attribution_diff'`. Distinct from `source_diff`: the two sides do NOT share a fine key (batch and/or block differ), only the weight agrees — so this is a same-fact PAIRING, not a same-key value disagreement. `row` jsonb: `{ transaction_date, destination, weight_kg, proposed: {source,batch,batch_code?,block_loc,weight_kg,provenance}, gsheet: {...} }`. Fingerprint = hash of `{date, destination, rounded weight, SORTED batch-id pair}` (order-independent). Resolution is **dismiss-with-note only in v1** — no pick-and-rewrite (the two sides don't share a natural key the existing diff-plan machinery can target).
...
| **R-ATTR — Second-pass attribution matcher** | A pure second pass inside the R1 engine that pairs two single-witness `weight_kg` facts (one proposed-only, one gsheet-only) on the same `(date, destination)` whose weight agrees within tolerance but whose batch/block attribution differs — the proposed report derives its batch from `(block_date, block_no)` while the Sheet carries an operator-typed code, so the SAME feeding resolves to two disjoint fine keys and would otherwise double-alarm as two `single_source_overdue` cases. Emits one `attribution_diff` case instead of two overdue cases; dismiss-only in v1 (no pick-and-rewrite). Motivated by the 2026-07-11 forensics on run `83d17774` (47 overdue cases ≈ 20 real pairs). | Shipped 2026-07-11; see `workers/sync/src/reconcile/CONTEXT.md` § "Second-pass attribution matcher". |
| **R-ALIAS — Patio block-name aliases** | A validated, hand-maintained table (`workers/sync/src/reconcile/blockAliases.ts`) mapping the PROPOSED report's DESCRIPTIVE sun-drying-patio spot names (e.g. `"16A NEAR WALL"`) to the Google Sheet's CODED equivalent (`"PCA-16A"`) — the Sheet's coded block is authoritative for these 8 seeded patio locations. Applied ONLY on the proposed side, ONLY at bucketing time (`rcOutStage.ts::bucketProposed`), before the fine-reconciliation key is built — so a proposed patio row now buckets under the Sheet's own coded key. When the two sides' batch also resolves to the same `batch_id`, this collapses what would have been an `attribution_diff` (or two independent `single_source_overdue` facts) into a normal same-key comparison: weights agreeing → a plain `Agreement`, weights disagreeing → a real `source_diff` (still surfaced, never auto-resolved). An unrecognized descriptive name passes through unchanged, so a non-patio attribution mismatch still falls to R-ATTR exactly as before. Reconciler-only — never writes, never touches `classify.ts`/the apply path. A run-summary telemetry line reports how many rows were auto-aligned this run. | Shipped 2026-07-13; see `workers/sync/src/reconcile/CONTEXT.md` § "Patio block aliases". |
