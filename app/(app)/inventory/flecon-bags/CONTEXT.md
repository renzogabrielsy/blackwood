...
## Key Behaviors
- **`fetchFleconBagData()` server action:** Returns `{ balances, movements, error? }`. Balances via `view_flecon_bag_balance` ordered by `sort_order`. Movements via a paginated `fetchAll<T>()` loop (1000-row `.range()` chunks) to bypass PostgREST's `max_rows` cap — mirrors the RC OUT pattern, now ordered **ASC** (`transaction_date` → `source_row` nulls-first → `created_at`). The join to `flecon_bag_types` is flattened defensively (object OR single-element array). One localized `any` on the `fetchAll` builder param mirrors RC OUT — the only `any` in the module. Graceful error: any query error returns `{ balances: [], movements: [], error }`.
