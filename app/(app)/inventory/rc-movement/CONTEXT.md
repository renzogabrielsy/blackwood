...
- **Frozen summary footer (bottom-pinned, mirror of the header) — THREE VALUES; WHOLE-CELL state tint; everything else on hover:** a sticky `<tfoot>` pinned to the container bottom (`bottom:0`). Each per-column cell is a tight stack (`px-2 py-0.5`, no inter-line gap) wrapped in a `Tooltip`/`TooltipTrigger` (reusing the table's existing `TooltipProvider`); the trigger content carries `cursor-default`.

  > **⚠ THE FOOTER ANSWERS ONE QUESTION — THIS CAMPAIGN'S (2026-09-17, second pass).** Earlier the same day it answered TWO and said which was which: Renzo had read `FED 69,013` on **AUGUST 2026 · JAN-26-BLK15** as the sum of the column above it — that column totals **54,941 kg** — and concluded the loss formula was broken. It was not; every figure on that footer except the first was the block's **LIFETIME** total (JULY 2026 opened that block and took the other **14,072 kg**). Labelling the two clocks fixed the misread and left five lines. Renzo, reading it: *"It is best that the footer remains just these values (top to bottom): kg fed (fed for the batch/campaign and not lifetime), php/kg (not actual), loss. The other values currently on that footer can be viewed on hover anyway."* He is right — the hover card already carried every one of them — so the lifetime half moved onto the hover card and the `title`, and **nothing was deleted**.
  - **Line 1 (FED — THIS CAMPAIGN):** a `flex justify-between` row — tiny `text-[9px]` uppercase `fed` label pinned left + **`column.campaignFedKg`** (bold `font-mono text-xs`, same weight/size as the frozen `Total fed` grand-total cell so the two read as one family) pinned right. **"—" when null, never `0`.** The `title` says *"Fed by this campaign"* in words, names the feed-day count and points at the hover card for the block's whole life. It equals the column's own cell sum **by construction** — both are `view_rc_movement_campaign_cells` (see "THIS CAMPAIGN's fed kg" below). Never re-added in TS.
  - **Line 2 (₱/kg — the block's DELIVERED weighted-average fed price, NOT the actual):** tiny `text-[10px]` uppercase muted `₱/kg` label + `column.avgFedPrice` (`fmtPrice`, `font-mono text-[10px]`). **Blank value when null**; the label slot is KEPT so every per-column footer stays the same height. Price-gated — the whole line is dropped for Production, which therefore reads a TWO-line footer.
  - **Line 3 (LOSS % ):** tiny `text-[10px]` uppercase `loss %` label + **`fmtLossPct(blockLoss)`**, `font-mono text-[10px]`. "—" when `blockLoss` is null. **NO LEADING MINUS ON AN ORDINARY LOSS.** `column.blockLoss` is the SIGNED ratio `(totalOut − totalIn) / totalIn`, so a block that lost weight carries a NEGATIVE ratio and the old `fmtSignedPct` printed `LOSS -0.86%` — which reads as "negative loss", the opposite of what happened. `fmtLossPct` (which REPLACED `fmtSignedPct`; there are no other callers) drops the sign: an ordinary loss prints **positive**, and the ~27% of blocks that fed MORE than was delivered keep an explicit minus, because they lost nothing. Same quantity SQL publishes as `view_rc_movement_block_actual_price.loss_pct`; that column is deliberately NOT read here because it rides on a **price-gated** view that is not queried at all for Production, and the loss figure must stay visible to every role. The red(loss)/emerald(gain)/muted(null) coloring is UNCHANGED (it keys off `blockLoss`, not the printed string).
  - **Line 4 (ACTUAL — ONLY WITH THE `Actual ₱` SWITCH ON, 2026-09-17):** Renzo: *"Being able to toggle on/off the ability to see the actual price of the block in the footer would be really cool."* A fourth line above a hairline, `actual` + `column.actualFedPrice` (`font-mono text-[11px] font-bold`). **BLANK when null** — an OPEN block, a closed block with an unpriced delivery, or one with a sun-drying outflow has no actual price; never ₱0.00 and never a dash that reads as a value (the avg_cost ₱11.01-vs-₱39.99 bug class), and the cell `title` names WHICH of the three reasons it is. Gated on `showActualLine = showFedPrice && showActualPrice`, so a stale `?actual=on` opened by Production reveals nothing. See "The `Actual ₱` switch" below.
  - **WHAT LEFT THE VISIBLE CELL AND WHERE IT WENT:** the `life` (lifetime outflow) line, the `all campaigns` caption and the always-on `actual` line. All three are on the hover card, which was already printing them — *measured on the fixture, the card reads `This campaign 54,941 kg · 4d | Fed (all camp.) 69,013 kg | In (all camp.) 76,013 kg | MC | Ash | Fed price | Actual fed | Uplift | Lost | Loss | Opened`*. The **phone summary** (`RcMovementSummaryMobile`) deliberately keeps both clocks: it has no hover to move them to and the room to print them.
  - **`W_BLOCK` WAS RE-MEASURED, NOT LEFT AT ITS OLD NUMBER: 124 → 116.** It had gone 92 → 124 that morning for the `this camp` label; with the footer cut, the widest line became `ACTUAL 1,234.56`. MEASURED in Chrome with `Range.getBoundingClientRect()` at the real computed fonts: `FED` 18.45 + 4 + `781,234` 46.83 = **69.28** · `₱/KG` 24.92 + 4 + `1,234.56` 40.98 = **69.90** · `LOSS %` 40.83 + 4 + `-10.56%` 34.71 = **79.54** · `ACTUAL` 42.26 + 4 + `1,234.56` 47.18 = **93.44**. The FIRST block column is the tight one (it also carries the 2px `GROUP_DIVIDER`), so its budget is `declared − 16 (px-2) − 1 (border-r) − 2 (divider)`: **116 buys 97px** against the 93.44 floor. **108 would have cleared the three default lines and WRAPPED the switched-on fourth** — which is how a width sized to the state you happened to be looking at regresses. The ₱ figures are measured at `1,234.56`, twenty times any real fed price. Pinned in `scripts/verify-rc-movement-grid.ts` §1b, for BOTH grids.
...
2. **`W_BLOCK`'s floor is the longest batch code that EXISTS** — 16 characters
   ...
   clears the 16-character floor there too, and it came back DOWN to **116** in the same
   day's second pass when the footer was cut to three values (four with the `Actual ₱`
   switch). The two still differ — 116 vs 148 — because the Classic `<th>` pays no
   header-chrome budget.
...
#### The per-block summary cell prints THREE VALUES (2026-09-17)

**Changed in LOCKSTEP with the Classic matrix's block footer, twice in one day** — the
misread was reachable on either grid, and the paramless URL serves THIS one, so fixing only
the Classic matrix would have left the default screen wrong. See "Frozen summary footer"
above for the incident (AUGUST 2026 · JAN-26-BLK15 printing `FED 69,013` over a column that
totals 54,941 kg), the labelling that answered it, and Renzo's follow-up (*"it is best that
the footer remains just these values"*). The cell is identical here, line for line:

| Line | Content |
|---|---|
| 1 | `fed` (`text-[9px]`, `shrink-0 whitespace-nowrap`) + **`block.campaignFedKg`** (`font-mono text-xs font-semibold`). **"—" when null, never `0`.** |
| 2 | `₱/kg` + `block.avgFedPrice` — the block's DELIVERED price, **not** the actual. Gated on `showFedPrice`. |
| 3 | `loss %` + **`fmtLossPct(blockLoss)`** — POSITIVE for an ordinary loss, **no leading minus** |
| 4 | `actual` + `block.actualFedPrice`, above a hairline — **ONLY** with the `Actual ₱` switch on (`showActualLine = showFedPrice && showActualPrice`) |
...
2. **`fmtSignedPct` is GONE from this file too** (it had exactly two callers, the cell and
   ...
   must not disagree about the sign of the same field, and the verify script asserts that
   on BOTH sources.
3. **`W_BLOCK` did NOT move — it is already 148** (sized for the HEADER, §"Header widths"),
   ...
   (border-r) − 2 (the first block column's GROUP_DIVIDER) = 129px` against the widest
   measured line, `ACTUAL 1,234.56` at **93.44px**. Pinned in
   `scripts/verify-rc-movement-grid.ts` §1b (`SUMMARY_ROW_PX` / `SUMMARY_CELL_CHROME`),
   which now checks **both** grids' declared `W_BLOCK` against the same floor. **The
   Classic matrix, whose `<th>` is cheaper, went 92 → 124 → 116** across the two passes.
4. **`TOTALS_H` came down 62 → 46**, and stays a FLOOR rather than a measurement: the same
   row is one line shorter for Production (no ₱/kg) and one line taller with the switch on,
   so a hard number leaves one of the three renders an empty band or a clipped one. The
   tallest cell decides, and the tricolor PRODUCED/YIELD/LOSS stack is what 46 is sized for.
5. **The `title` carries everything the cell stopped printing**, and MORE than it did: it
   leads with `This campaign fed N kg over M days`, then `Whole life — out … · in …`, then
   MC/Ash/Loss, then **the ACTUAL price whether or not the switch is on** (naming which of
   the three reasons a blank one is blank), then `Opened`. The footer's fourth line is a
   convenience; this is the fallback.

#### The `Actual ₱` switch — ONE component, THREE hosts (2026-09-17)

Renzo: *"Being able to toggle on/off the ability to see the actual price of the block in the
footer would be really cool."* `actual-price-toggle.tsx` is the control, and it is a FILE
rather than three buttons because three surfaces show a block footer — this grid, the
Classic matrix on `?grid=v1`, and `/operations`' RC FED modal, which HOSTS the Classic
matrix. It exports `ActualPriceToggle`, `ACTUAL_PARAM` (`actual`), `ACTUAL_ON` (`on`) and
`parseActualPrice`.

- **It holds NO router hook, deliberately.** The Classic matrix must stay prop-driven (see
  "Consumers outside this route"), so the switch is a controlled button and the HOST owns
  where the state lives.
- **WHERE THE STATE LIVES, per host.** This grid reads `?actual=` out of the `searchParams`
  it is already given and writes it with the same param-preserving `router.replace` its
  campaign picker uses. `rc-movement-route-view.tsx` reads it with `useSearchParams` and
  passes `showActualPrice` + `onShowActualPriceChange` into the matrix, which then renders
  the switch in its OWN toolbar. `/operations`' modal passes `showActualPrice` from React
  state and **no callback**, so the matrix renders no control and the modal puts the
  identical component beside its campaign tabs — *measured on the fixture: exactly ONE
  toggle in that composition.*
- **OFF is the default and is spelled as ABSENCE**, the contract `?campaign=` and `?grid=`
  already keep on this route.
- **ABSENT, never disabled, for a price-denied reader.** `fetchRcMovementMatrix` does not
  even QUERY the three actual-price views for Production, so there is no actual price to
...
- **`/operations` (the Plant Operations Ledger) renders the CLASSIC matrix in a modal
  ...
  **2026-09-17: the matrix gained TWO OPTIONAL PROPS and this consumer uses one of them.**
  `showActualPrice?: boolean` (default FALSE, so no existing host changed behaviour) and
  `onShowActualPriceChange?: (next: boolean) => void`. The modal passes the FIRST from its
  own React state and **not the second**, so the matrix renders no control and the modal
  puts `ActualPriceToggle` beside its campaign tabs instead; the route host passes both and
  keeps the state in `?actual=`. The state is local there on purpose — `/operations`' address
  describes which campaigns and which lens the LEDGER shows, and a disclosure inside one
  dialog is the same category as the expanded day and the block drawer, which that screen
  also keeps out of the URL. The modal also inherits the trimmed three-line block footer for
  free (no edit there).
