...
| `../_shared/blend-market-chart.ts` | **NEW (2026-09-26). PAGE ONE'S MARKET CHART on the blend print + PDF** — pure, no React. `buildBlendMarketChartModel` (the normalized `BlendMarketChartModel` — four series, two axes or one, legend, footnotes), `layoutBlendMarketChart` (THE geometry, in any unit — px for the SVG, pt for jsPDF with its own text measure), `blendMarketChartSvg` / `buildBlendMarketChartSection` / `buildBlendMarketUnavailableSection` / `buildBlendMarketSlotHtml`, `BLEND_MARKET_CHART_PRINT_CSS` (incl. the `.p1` page-one wrapper) and `blendMarketPlotHeightPx`. `BlendMarketChartSlot` = `{kind:'chart', model}` or `{kind:'unavailable', reason}` — BOTH documents consume the same slot. **Computes no statistic**: every figure is a field of `fetchBlendMarketHistory`; the only arithmetic is kg → tonnes and axis geometry. See "PAGE ONE'S MARKET CHART" below. **Since 2026-09-26 it also serves the ON-SCREEN chart** (one model, one geometry, THREE thin renderers — print SVG string, jsPDF, React SVG): every `BlendMarketTextItem` carries a `role` (`leftTick · leftUnit · rightTick · rightUnit · month · partial · ref`, + `slot` on month labels) so the screen can ink by meaning; the model gained `monthLabels` (`Oct 2025` …); new pure helpers `blendMarketSubtitle(showPrices)` (the definitions line, now the model's own source), `blendMarketSlotAt(layout, x)` (crosshair hit-test), `blendMarketTooltip(model, i)` (formatting only — ₱/kg 2 dp, tonnes 1 dp, `null` = "no data", never 0) and `BLEND_MARKET_SHORT_LABEL`. **None of it reaches the printed SVG string** — the print is byte-identical (measured against the pre-change output: HTML slot + jsPDF, four models + the unavailable note). |
| `../_shared/blend-market-chart-screen.tsx` | **NEW (2026-09-26). THE BLEND DIALOG'S ON-SCREEN MARKET CHART** — `BlendMarketChartScreen`, a `'use client'` React SVG, the third renderer of the model above. Fetches nothing: the dialog passes the SAME `marketSlot.model` it prints. Theme-token ink (text/axes/grid) + the four hues tuned per theme (700 light / 400 dark); ResizeObserver width (labels stay 10 px; month labels thin on a narrow box, anchored on the newest); crosshair tooltip (pointer, tap, ←/→/Home/End, Esc); legend toggles; fixed 220 px plot box; loading skeleton at the same height; inline `RefusalBanner` (Copy + Retry) on error and after a 15 s stall; collapsible, open state remembered per viewer in `localStorage` (`bw.blend_market_chart.open`, try/catch). See "ON SCREEN" under "PAGE ONE'S MARKET CHART". |
...
**Look rig:** `/dev/table-playground/blendanalysis?market=gaps|fail|throw|stall|flaky` (the
...
#### ON SCREEN — the same chart in the dialog body (2026-09-26)

Renzo: *"The PDF looks great. Now add that chart to our web app, in the blend proposal
section."* `BlendMarketChartScreen` (`_shared/blend-market-chart-screen.tsx`) renders the
chart inside `BlendProposalDialog`, for the live what-if AND a saved version.

- **Placement.** In the scrolling body, **after the Pricing block and before Selected
  Blocks** — the print's own order (Summary → Blended Lab Stats → Pricing → chart). A
  heading row (`RC MARKET — LAST 12 MONTHS` + the range) is a **collapse toggle**, open by
  default, remembered per viewer in `localStorage` (`bw.blend_market_chart.open`, every
  access in try/catch; a blocked store just forgets). Collapsed, the blocks table sits where
  it did before the chart existed. The dialog has no tabs; this is the first collapsible
  section, and it collapses to its heading only.
- **ONE fetch, ONE model.** The dialog's existing `marketRead` (one
  `fetchBlendMarketHistory` per open / version / retry) builds `marketSlot` once; the
  screen gets `marketSlot.model` — the object the printout and the PDF draw. A saved version
  is therefore anchored on its `asOfAt`, exactly like the print. **Retry** bumps
  `marketNonce`, which is part of the read's signature (`marketLoadKey`), so a late answer
  to an abandoned read is dropped.
- **ONE geometry.** It calls `layoutBlendMarketChart` at the plot box's REAL pixel width
  (ResizeObserver) and a fixed **220 px** height, so the scales, runs, gap breaks, partial
  hollow points, dual ladders and the blend's dashed reference are the print's own. Text is
  never viewBox-scaled (10 px stays 10 px); on a narrow box the month labels are thinned to
  every k-th, anchored so the newest (partial) month always keeps its label.
- **Ink.** No print ink and no hex anywhere: labels by `role` → `fill-foreground` /
  `fill-muted-foreground`, guides `stroke-border`, baseline `stroke-muted-foreground`,
  hollow points `fill-background`; the hues are `blue/orange-700` lines and
  `purple/green-500` areas (22% fill, 700 edge) on light, all `-400` on dark.
- **Interaction.** A crosshair column + ringed points + a tooltip for the hovered month
  (pointer, tap, or focus the plot and use ←/→/Home/End; Esc closes the tooltip, not the
  dialog): `₱/kg` to 2 dp, tonnes to 1 dp, `partial` on the current month, and **"no data"
  for a gap, never 0** (`blendMarketTooltip`). The legend toggles each series (and the
  reference line) on and off; axes do not rescale. No animation beyond the 150 ms chevron.
- **Price gate.** Rendered from the server-nulled model: a price-denied reader gets the two
  volume areas, ONE axis (`t / month`), a two-entry legend and a two-row tooltip — no ₱ in
  the section at all. `showPrices` only predicts the legend while the read is loading.
- **States.** `loading` — a skeleton in the fixed plot box with the legend, the definitions
  line and one reserved footnote line already laid out, so the section is the SAME height
  loaded (measured, < 1 px). `error` — an inline `RefusalBanner` with **Copy** and **Retry**
  (the error HARD RULE, banner form); nothing else in the dialog changes. A read that never
  answers keeps the skeleton and adds the same banner after 15 s.

**Proofs:** `npx tsx scripts/verify-blend-market-screen.ts` — static (the screen fetches and
folds nothing and uses the shared geometry; the dialog hands it the SAME `marketSlot.model`
from the ONE read; no hex/black/white ink; the fixed-size classes match their constants),
pure (tooltip values and "no data", the price-denied rows, the hit-test, text roles never
reaching the printed SVG) and a browser half on the rig: placement between Pricing and
Selected Blocks, four series vs two for `?prices=0`, the tooltip's four values on Sep 2026
and "no data" on the April gap, legend toggling, loading height = loaded height, the error
banner's Copy + Retry for `fail`/`throw`, Retry recovering on `?market=flaky`, dark-mode ink
= theme tokens only, and at 375 px no horizontal page scroll with thinned month labels.
`BLEND_MARKET_SHOTS=<dir>` also writes light + dark screenshots of the dialog.
`verify-blend-market-chart.ts` still proves the printed page one unchanged.
