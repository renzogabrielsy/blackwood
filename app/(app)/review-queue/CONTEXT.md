...
- `components/review-queue/ReviewDetailPanel.tsx` — Fetches `getReviewDetail(id)` on mount; sticky glass header (confidence + report type + filename), diagnostic banner, classified rows table, sticky footer with Approve / Reject. **Both Approve and Reject open a confirming `AlertDialog`** (approve confirm states the exact insert/change-decision counts written to `deliveries` — irreversible; reject takes an optional reason). The footer's Approve/Reject button group is **desktop-only** (`hidden sm:flex`, per the locked mobile scope "approve/reject/upload stay desktop"); below `sm` a muted inline note ("Review actions are available on desktop.") renders in its place. Errors use `errorToast()`
- `components/review-queue/ClassifiedRowsTable.tsx` — **Responsive (Archetype C).** Desktop (`hidden sm:block`): Excel-dense `table-fixed` (~1575px) with explicit column widths matching RC IN canonical order; per-row decision toggle column; changed cells get a left amber border + dual-value display (email bold on top, DB struck-through below). Mobile (`sm:hidden`): a phone card list on the platform `MobileCardList<ClassifiedRow>` primitive, fed the SAME `rows` + `COLUMNS` as the desktop table (single source of truth — no second data path). Card headline (≤6): confidence dot · supplier · weight · date · batch · New/Changed badge, with changed rows flagged by an amber left rail. Tap → bottom Sheet detail rendering **every** `COLUMNS` field; changed cells reuse the desktop dual-value treatment (email value bold on top, DB value struck-through below) inside an amber-tinted field. The per-row decision is **read-only** on mobile (`DecisionStatus` — a static chip mirroring `ROW_DECISION_OPTIONS` labels, or "Auto-insert on approve" for NEW rows); review **editing** stays desktop-only per the locked mobile scope. Local sub-components: `ClassifiedRowsCards`, `RowCard`, `StateBadge`, `RowDetail`, `DetailField`, `DecisionStatus`
- `components/review-queue/RowDecisionToggle.tsx` — 3-state segmented control (`email_wins` / `db_wins` / `both`); hand-rolled on buttons because the project has no `ToggleGroup`/`RadioGroup` primitive. Exports `ROW_DECISION_OPTIONS` (value/label/tooltip) so the mobile read-only `DecisionStatus` reuses the same labels
