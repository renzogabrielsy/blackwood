...
| `ledger-derive.ts` | Pure `deriveDailyMetrics(row: GridRow)` — now shared by THREE surfaces (the live grid's inline compute, the mobile card, and `daily-grid-v2.tsx`) — the DT TTL / PROD HRS / PROD LOSS / TTL WASTE formula (shift default 8h) in ONE place, mirroring the grid's inline compute. Consumed by the mobile card so both surfaces show identical derived values. **The 8-hour default now MATCHES the sync (2026-09-14, L-051)**: `production_downtime.shift_hrs` used to be the literal `12` on every row the sync wrote, so PROD HRS on screen (`8 − DT TTL`) and the stored shift length disagreed by four hours on every single day. The extractor now DERIVES it — 12 h only when the day carried an overtime signal, 8 h otherwise — and records why in `shift_hrs_source`. ⚠️ **Still open: which number is right.** 158 rows backfilled from Renzo's `MASTER ICTC INPUT FILE V1.xlsx` (2025-11-27 … 2026-05-23) carry **9**. If 9 is the true day shift, the constant HERE and `DEFAULT_SHIFT_HRS` in `workers/sync/src/reports/production/shiftHours.ts` move together — they must never drift apart again. |
