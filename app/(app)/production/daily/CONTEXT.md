...
| `ledger-derive.ts` | Pure `deriveDailyMetrics(row: GridRow)` — now shared by THREE surfaces (the live grid's inline compute, the mobile card, and `daily-grid-v2.tsx`) — the DT TTL / PROD HRS / PROD LOSS / TTL WASTE formula (shift default 8h) in ONE place, mirroring the grid's inline compute. Consumed by the mobile card so both surfaces show identical derived values. **A NORMAL SHIFT IS NINE HOURS and this file now OWNS that number (2026-09-14, L-051/b).** It exports `DEFAULT_SHIFT_HRS = 9` (Renzo: 08:00-17:00 with an hour off, which is what all 158 rows of his `MASTER ICTC INPUT FILE V1.xlsx` already said). Before this, the literal `8` was written out in FOUR places on the app side — here, the desktop grid's inline cell compute, that grid's footer aggregate, and `daily-grid-v2-save.ts`'s `ASSUMED_SHIFT_HRS` — plus `actions.ts`'s `?? 8`, while the SYNC stored `12` on every row it wrote, so PROD HRS on screen and the stored shift length disagreed by four hours every day and nothing could see it. All of them now read this constant, and `DEFAULT_SHIFT_HRS` in `workers/sync/src/reports/production/shiftHours.ts` — the number that gets STORED — is pinned to it by a worker test that reads THIS FILE'S SOURCE TEXT (never an import: the worker may not depend on `app/**`). 12 h is an overtime day, and the sync records which rule fired in `production_downtime.shift_hrs_source`. |
...
| Downtime | DT HRS / DT MIN / DT TTL (computed) / PROD HRS (computed) / DT REASON (120px, message-icon → Popover textarea). **The stored DT HRS / DT MIN moved on 2026-09-14 (L-051)** — they now come from MC's own list of stop-and-start times instead of her hand-written DURATION total, which she stopped filling on 2026-08-01. August and September had been reading `0.00` downtime; the backfill took 2026-05 → 2026-09 from 10.55 h to 51.18 h across 62 rows, then L-051b's re-run settled it at 38.33 h by recording 2026-08-11's `8:00-11:37` as an INCIDENT — a range whose reason says the plant did not stop is trouble production ran through, not downtime, and it contributes 0 while staying visible in `dt_incident_ranges`. Nothing on this page changed; the numbers underneath it did. |
