...
| `summaries-client.tsx` | `'use client'` shell. URL-driven view toggle via `?view=period\|supplier` (default `period`; `?view=period` deletes the param to keep the URL clean). Renders `AnalystBriefClient` (period) or `SupplierBriefClient` (supplier). `useSearchParams` is wrapped in a Suspense boundary. **Pending UI:** the `?view=` write is wrapped in `useTransition` (this route is dynamic, so it re-runs BOTH server analytics fetches — seconds of silence before). The rendered surface is driven by a `useOptimistic` mirror of `view`, NOT the URL value — both payloads are already in props, so the switch lands on the click's frame; the in-flight navigation only refreshes those props and is signalled by a `Loader2` in the active segment. |
| `loading.tsx` | Route-level skeleton (toggle strip + year rail + totals + monthly table). Static pulses only — no row animation. |
