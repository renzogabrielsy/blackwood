...
Note `sync_gsheet.py`'s legacy `_apply_from_compact` uses plain `db.insert()` (NOT `insert_if_absent`) for both `deliveries` and `rc_out` NEW rows (sync_gsheet.py:362, 386) — idempotency there is enforced upstream by the classifier's NEW/NOOP decision against a freshly-queried DB window, not by a second per-row re-check at write time.

> **The TS port NO LONGER preserves this asymmetry (2026-07-27, BUG-016).** `src/reports/gsheet/apply.ts` now calls `insertIfAbsent` for both tables, reusing the SAME natural keys listed in the table above (`deliveries` from sync_deliveries.py:343, `rc_out` from sync_rc_out.py:265) — exported as `GSHEET_DELIVERIES_NATURAL_KEY` / `GSHEET_RC_OUT_NATURAL_KEY`. The upstream-only guard was unsound because gsheet is NOT the sole writer of `deliveries`: the classifier's start-of-run snapshot is a TOCTOU window, and on 2026-07-15 it let gsheet blind-insert a duplicate of a delivery the email path had written 2h07m earlier (+24,024 kg phantom inventory). A guard hit surfaces as a held row with `reason`/`kind` `already_exists` — the same words the email writers use. See `gsheet.md` §5 "Idempotency mechanism" and PORTING_DECISIONS.md.
...
9. **A number interpolated into a MESSAGE STRING is not covered by the canonicalizer, and the two engines render it differently.** The parity canonicalizer tags and normalizes numeric *values*, so `weight_kg: 18827` compares fine — but a number formatted INTO a string is just text. Python `str(18827.0)` is `'18827.0'`; JS `String(18827)` is `'18827'`. This has now bitten twice: the deliveries `dup_noop` `natural_key` (fixed with a `pyFloatStr` on the TS side, matching Python) and the L-040b `identity_diff` refusal text (fixed the other way — a `_js()` renderer on the PYTHON side, so the human-readable message reads `18827` rather than `18827.0`). **Rule:** pick a direction per message, write the helper on the side that has to bend, and never assume a `reason`/`note` string is parity-safe just because its inputs are numbers.
10. **`identity_diff` / two-tier delivery identity (L-040b) is IMPORTED, not re-derived.** `classify_gsheet.py` imports `build_identity_index` / `match_delivery` / `MUTABLE_IDENTITY_FIELDS` from `classify_deliveries.py`, and the TS side has one `src/lib/deliveryIdentity.ts`. If you are porting or editing either classifier, do NOT add a local copy of the key — two writers of `deliveries` with two identities duplicate each other's rows, which is the bug the whole change exists to remove.
