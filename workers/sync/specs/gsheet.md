...
2. NEW rows (RC IN): defensive batch upsert (catches `is_location_collision` → `skipped`, not `held` — note gsheet's legacy result shape uses the key `skipped`, NOT `held`, for this concept; the CONTRACT wrapper `phase_apply_contract` RE-MAPS these into the contract envelope's `held` list with `reason:"skipped"`), then `db.insert()` (**PLAIN insert, NOT `insert_if_absent`** — see porting trap below), then `update_trigger_audit_provenance` with a note about the `cost_basis=0` placeholder (L-008) if unenriched.
3. NEW rows (RC OUT): `db.insert()` (also plain, not `insert_if_absent`), then `insert_manual_audit` (no trigger on rc_out).
...
6. `unmapped` rows: `decision == "skip"` (default) → skipped with "never auto-create a batch" note; any other decision value → skipped with a note that it requires re-classification with the corrected batch_code (never auto-applied either way).
...
### Idempotency mechanism — DIFFERENT from every other pipeline

**`sync_gsheet.py`'s `_apply_from_compact` uses PLAIN `db.insert()`, NOT `db.insert_if_absent()`, for BOTH `deliveries` and `rc_out` NEW rows** (verbatim: lines 362, 386). This is a deliberate divergence — idempotency here is enforced entirely UPSTREAM, by the classifier re-querying the DB fresh every classify call and only routing genuinely-absent rows into `new`. A TS port must NOT "helpfully" add a re-check-before-insert here; doing so would change behavior from the documented/intended design (though arguably it would ALSO fix a latent race-condition risk — flag this as a possible improvement opportunity, not a required parity target, since L-020's idempotency rule was explicitly scoped to deliveries-manager/rc-out-manager EXECUTE, not gsheet-sync, per the ledger).
...
| never-auto-create-batch | `_apply_from_compact` (only ever upserts the ALREADY-resolved code) | No code path derives a batch_code from scratch here. |
...
4. **Idempotency is intentionally NOT enforced at insert time** (plain `db.insert()`, not `insert_if_absent`) — do not "fix" this without an explicit human decision, since it's a documented (if debatable) design choice distinguishing gsheet from the other three sync-writer pipelines.
