...
8. **Classify**: builds `batch_lookup.json` (`{batch_code: id}` for ALL batches, no date filter — `since_column=None`), fetches `rc_out` DB rows over the `since` window, runs `classify_rc_out.py` passing `--watermark {watermark}` IF a watermark exists (omitted only for a fresh/empty table — a genuine first-run backfill).
...
| `production_batch` | `norm_str` equal, null≡empty |
...
No materiality/immateriality gate beyond what `norm_*` collapses — every surviving diff is material (unlike gsheet's `is_material`).

### FLAGGED kinds — exact trigger conditions

1. **Sub-watermark write guard** (L-019, classify_rc_out.py:236-252): a row with NO natural-key match (`matches` empty) where `watermark is not None` AND `ex_row["transaction_date"] <= watermark` → routed to `flagged`, NEVER to `new`, regardless of how confident the extraction is. This is a HARD string comparison on ISO date strings (`"2026-05-29" <= "2026-06-16"` — lexicographic, which works correctly for zero-padded ISO dates).
2. **UNMAPPED** (lines 218-227): `resolve_batch_id` returns `(None, ...)` — no primary or fallback code resolves to a `batch_id`.
3. **MALFORMED** (lines 208-214): missing `transaction_date`, OR `weight_kg`/`day_total_kg` is `None` or exactly `0` (note: `float(w) == 0` — a section whose day total genuinely IS zero, e.g. a block that received nothing that day, is treated as malformed/dropped, not a legitimate zero-weight NOOP).
