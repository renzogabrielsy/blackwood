...
| `schedule-preview-mobile.tsx` | `'use client'` | `schedulePreview` | **Phone-only** condensed schedule view (`sm:hidden`). A compact stacked list of the nearest **5** days rendered via the shared `ScheduleRowCard` + a **"View full table" bottom `Sheet`** (`side="bottom"`) rendering the full `ScheduleTable` (`min-w-[640px]`, sideways-scroll inside the sheet). No ₱ → no gating. |
