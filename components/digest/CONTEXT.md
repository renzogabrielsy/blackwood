...
| `schedule-table.tsx` | — (client-safe, no `'use client'`) | — | The shared **dense schedule table** extracted from `schedule-preview.tsx` so BOTH the desktop card AND the mobile bottom sheet render identical markup. Columns: Date · Day · **Setup / grades** · Sh · **Total t** · Act t · **Act hrs** · Status · Src. Setup cell stacks per-grade tonnage (`parseGradeTons`/`fmtGradeTons`/`gradeTonsTitle`); today's row accent-tinted, rest days dashed; Status/Src chips as before. Props: `rows`, `maxHeightClass` (scroll cap), `minWidthClass` (keeps column widths so the wrapper scrolls sideways instead of crushing). Also exports `fmtTons`. No server-only imports (importable by the client mobile component). |
| `schedule-preview-mobile.tsx` | `'use client'` | `schedulePreview` | **Phone-only** condensed schedule view (`sm:hidden`). A compact stacked list of the nearest **5** days (date · setup/grades · tons · status chip) + a **"View full table" bottom `Sheet`** (`side="bottom"`) rendering the full `ScheduleTable` (`min-w-[640px]`, sideways-scroll inside the sheet). No ₱ → no gating. |
...
- **`digest-charts.tsx`** — `ChartCard` has a phone-only expand button
  ...
  reused in both spots; the sheet chart mounts only while open).
...
- **`trucks-summary` / `bag-inventory` / `sync-summary` / `activity-feed` /
  ...
  `flex-wrap` / stacking grids); no condense/expand needed.
