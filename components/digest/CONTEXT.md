...
| `schedule-month-view.tsx` | **Server (async)** | — (queries directly) | The full **month plan-vs-actual** schedule — the ex-`/production/schedule` page body, moved into the digest world (BUG-003: it wrongly inherited the production tab shell). Rendered by BOTH entry points — `/` under `?view=schedule` AND the standalone `/production/schedule` route — in the SAME `HOME_SHELL_CLS` container (`shell.ts`), with the same data loading. **Two doors, one surface; no fork.** (BUG-003 was the production TAB SHELL leaking onto the schedule, not the URL; that shell now lives in the `app/(app)/production/(tabs)/` route group which the schedule route sits outside of.) **Domain layer, not the digest adapter** — it queries **`view_production_schedule_state`** (the ownership-aware read model) + **`view_production_schedule_conflicts`** + `view_digest_prod_actual_tons` + `view_digest_daily_hours` DIRECTLY (aggregation still in SQL; the month-total footer is a display sum). It **fetches and shapes only** — rows go to the `'use client'` `ScheduleMonthGrid`, which owns all interaction; the client never touches Supabase. It also loads the **setup library** (`production_setups`, `active = true`, in `sort_order`) in the same `Promise.all` and passes it down as `setups`: the grid must project as the operator types, and a round-trip per keystroke is not a design option. Retired setups are absent by design — still valid on every historical plan row, still visible at `/production/setups`, just not pickable. Renders NO page container — the host owns the shell. Props: `month` (`?month=YYYY-MM`, validated; defaults to the operational month), `basePath` + `extraParams` → the prev/next month hrefs (`/` passes `{ view: 'schedule' }`). Also renders the month-nav bar, the 4-swatch owner legend (Joseph / Sheet / You / **Actual (reported)** — no longer "Actual (frozen)", which was exactly the conflation that locked the days) and an amber "N pending upstream changes" chip when the month has parked proposals. Phone (`sm:hidden`): `ScheduleCardsMobile`, READ-ONLY, with an honest "read-only on this screen — editable on a tablet or desktop" note (the ONE place editing genuinely does not exist, so the hint names the screen size instead of pointing at a grid that is not rendered). No ₱ → no gating. |
| `schedule-month-grid.tsx` | `'use client'` | — | **The editable month grid (Phase B).** Built on the shared **Blackwood Table** primitives (`GridCell`, `EditInput`, `useGridEditSession`, `useGridKeyboardNav` + `createCoordinateNavResolver`) — no second editing engine. Columns: Date · Day · **Setup** · Grades · **Shifts** · **Proj t** · Act t · Act hrs · Var · Status · **Owner** · **Remarks** · actions; the four bold ones are editable (`SCHEDULE_COLUMN_MAP`). `table-fixed` + explicit px widths summing to `min-w-[1362px]` inside `overflow-x-auto` ("never crush, always scroll"). Edits are staged as per-day **drafts**; each dirty day saves via one `saveScheduleDay` call carrying the `row_version` that row was READ with. **Ownership legibility BEFORE commit:** the Owner chip previews the flip (`Sheet` struck-through → `You?` in sky with a ring), the row gets a sky left rail + tint, and a sticky glass save bar names every day whose ownership is about to move ("Take ownership & save N"). **Editability affordance:** every editable GridCell carries `cursor-cell` + `EDITABLE_CELL_HOVER_CLS` (`hover:bg-sky-500/10` + `hover:ring-1 hover:ring-inset hover:ring-sky-500/40`, `transition-colors duration-150`) — reusing the house `cursor-cell` idiom and `DatePickerCell`'s tint-on-hover, NOT a per-cell pencil icon (31×4 icons of noise). The hover half is suppressed on the ACTIVE cell so it can never out-specify that cell's `ring-2 ring-primary` selection ring. A leading muted hint line names the editable columns, the click/type · F2 · Enter · Esc model and the picker behaviour BEFORE the ownership warning, and links to `/production/setups`. **EVERY DAY IS EDITABLE, including reported ones (2026-07-30, migration `20260730090000`)** — the old `isScheduleDayEditable`-gated padlocked `<td>`s are gone; reportedness now shows as an emerald `CircleCheck` on the date cell + the emerald `Actual` chip + a sky `corrected` badge from `humanEditAfterReport`, and the first edit of a reported day in a browsing session raises a one-time `AlertDialog` (module-level `reportedEditAcknowledged`, because month nav is a `<Link>` and remounts the grid). **THE SETUP PROJECTION** (the "changing setup doesn't compute the grades" fix): Setup is a dropdown (`ScheduleSetupCell`) over the ACTIVE library, and the recompute rule is stated once, in this file's header — **(A)** picking a library setup ALWAYS recomputes `grades` + `projected_tons` at the day's current shift count; **(B)** changing `shifts` recomputes ONLY if the day was still `isOnTemplate()` at the OLD shift count, so a deliberate override survives; **(C)** "— No setup" and re-picking a legacy (not-in-library) string never recompute. Grades (col 3) stays read-only but renders LIVE from the projection draft; `projected_tons` stays freely editable and an off-template day is badged `*`, never blocked. `+ New setup…` opens `SetupFormDialog` → `createProductionSetup` → applies to the day in the same motion. Math is imported (`projectSetupByCode` / `isOnTemplate`), never reimplemented. **Revert:** human-owned days get a one-click `RotateCcw` → `AlertDialog` → `releaseScheduleDay` (reported days included — release is inert on them and withholding it would rebuild the one-way ownership ratchet). **Conflicts:** an amber marker in the Owner cell opens `ScheduleConflictDialog`. Errors always via `errorToast()` (persistent + Copy), and a failed save's typed values are embedded in the toast because the draft is discarded. Motion: container `animate-fade-up` only; rows never animate (`transition-all duration-150` hover). |
| `schedule-setup-cell.tsx` | `'use client'` | — | **The Setup cell — a library dropdown, not free text.** Modelled on `components/shared/grid/SelectCell` (same DropdownMenu primitives, same drag-suppressing mouse/pointer-down guards) but deliberately NOT that component, because a setup picker needs three things a generic categorical cell must not grow: a `+ New setup…` **action** at the bottom (opens a dialog rather than setting a value), a per-option `t/shift` hint (`projectSetup(mix,1)`, the one implementation), and a pinned **legacy row** for a stored setup string absent from the library — `production_schedule.setup` is free text with no FK, so a day can carry a name that was never added or has since been retired, and dropping it would make the cell read empty and let the first pick destroy a real value. `open` is CONTROLLED by the grid so F2 / Space on a selected setup cell opens the menu exactly where other columns open an inline editor. `SelectCell` stays platform-layer and untouched; this is tenant-layer. |
| `schedule-conflict-dialog.tsx` | `'use client'` | — | Arbitration for a day whose upstream change the sync withheld. Renders a **precise field-by-field diff** (Field \| Yours \| Joseph proposes) driven by `view_production_schedule_conflicts.changed_fields` — never two opaque JSON blobs. Two outcomes, the ONLY callers that pass `clear_pending: true`: **Take Joseph's** (`takeUpstreamProposal` — writes his values + clears the pending; the day stays human-owned and the sync's own rule-4 *reclaim* hands ownership back next run because the values now match) and **Keep mine** (`keepMineClearPending` — clears the pending, values untouched). **`grades` JOINED the `WRITABLE` set (2026-07-30)** once the setup library gave the app a writer for it, so a grades difference is now carried across like any other and the dialog no longer apologises for dropping it; the leftover `readOnlyOnly` copy covers the rare case where only `source` (provenance, never a plan value) differs. |
| `schedule-types.ts` | — (pure) | — | The row contract shared by the server view and the client grid: `ScheduleGridRow` (plan + actual + `owner`/`effectiveOwner`/`isReported`/`humanEditAfterReport`/`rowVersion`/`conflict`, plus **`grades`** — the keyed `GradeMix` `isOnTemplate` compares against, alongside the display-sorted `gradeTons`), `ScheduleConflict`, `ScheduleConflictSide`, `ScheduleEditableField`, `SCHEDULE_COLUMN_MAP` (visual col → editable field; **must stay in lockstep with the grid's `<th>`/`<td>` order**; `setup` at col 2 is in the map so nav REACHES it but the grid overrides `isEditable` for it, routing F2/Space to the dropdown), `scheduleFieldToString`. |
| `schedule-owner.ts` | — (pure) | — | Ownership vocabulary + presentation tokens mirroring the DB CHECK on `production_schedule.owner`: `ScheduleOwner`, `toScheduleOwner` (narrows an untrusted DB string), `OWNER_LABEL` / `OWNER_CHIP` / `OWNER_HINT` (the hover sentence explaining why the row behaves as it does), plus `REPORTED_HINT` and `HUMAN_EDIT_AFTER_REPORT_HINT`. Colour idiom follows `status-tokens.ts` — violet Joseph, muted Sheet, **sky** You (amber is already "today/awaiting"), emerald Actual. **`isScheduleDayEditable` is GONE (2026-07-30)** — deleted rather than changed to `return true`, because a constant predicate is an invitation to re-introduce the bug it caused: it returned false for `'actual'` and put 166 of the calendar's 273 days out of reach in-app. **Editability and reportedness are independent facts now; do not re-conflate them.** |
...
- **"Older than expected" is `missedDays`** = planned WORKING days
  (`production_schedule.shifts > 0`) strictly between `through_date` and the
  operational date. Rest days and the operational date itself are excluded, so a
  Sunday is never late and today's not-yet-due report is never late. Counted in
  SQL (`view_digest_stream_status.missed_working_days`).
...
- **`awaiting` is deliberately NOT emitted for lag-by-design streams.** "Today
  ...
  entire working day — the exact bug. It stays live for same-day streams and for
  `ScheduleRowState` (week strip / schedule tables).
...
| File | Client? | Role |
|------|---------|------|
| `lib/digest/day-status.ts` | pure | The state resolvers — `resolveKpiDayStatus()` → `reported`/`awaiting`/`rest`/`stale`/`idle` (rc_in = procurement → `idle`, not late; net_flow stays neutral) + `resolveKpiAnchorDate()` / `resolveKpiPrevDate()` / `streamForKpi()` (which DAY a card's number belongs to) + `resolveScheduleRowState()` for the week strip. Exports the `LATE_AFTER_MISSED_DAYS` (1) / `OVERDUE_AFTER_MISSED_DAYS` (2) thresholds. Also **owns the `ProdSchedDay` / `PlannedShifts` type** (moved here when the frozen `prod-schedule-draft.ts` constant was retired — the plan now comes from the `production_schedule` table via `getDigestData()`). **Where the logic lives:** every ROW-SET fact (latest/previous reported day, missed-working-day count) is computed in SQL; this module only BRANCHES over those scalars. Do not reintroduce a TS scan of the daily series to find "the latest day with data". |
...
`w-auto` Setup column is what crushed); `schedule-month-grid.tsx`
**`min-w-[1362px]`** (every one of its 13 columns has an explicit px width and the
min-width is literally their sum — the Setup column carries +22px for the dropdown
chevron so `3X50 / 8X50` still reads, and the sum moved with it); `trucks-summary` `min-w-[320px]` (200 + 120
...
- **Schedule editor only** (these three are the tenant-layer plan module reaching in — no other band imports them):
  - `@/lib/production/setup-projection` — `projectSetup` / `projectSetupByCode` / `isOnTemplate` / `parseGradeMix` / `toProductionSetup`. **PURE, and the ONE implementation of the projection.** Never reimplement the math here; `scripts/verify-setup-projection.ts` fails if a second copy appears.
  - `@/app/(app)/production/schedule/actions` — `saveScheduleDay` / `releaseScheduleDay` / `takeUpstreamProposal` / `keepMineClearPending`.
  - `@/app/(app)/production/setups/actions` (`createProductionSetup`) + `@/components/production/setup-form-dialog` — the inline "+ New setup…" path, shared verbatim with the `/production/setups` management screen so the two can never disagree about what a setup is.

## See Also
- [Home Daily Sync Digest](../../app/(app)/CONTEXT.md) — the page shell + full `DigestData` contract and per-band data notes.
- [Production module](<../../app/(app)/production/CONTEXT.md>) — the schedule's server actions, the ownership model, and the setup library / projection rules.
