...
| `schedule-preview.tsx` | Server | `schedulePreview` | Dense Excel-Standard **Production Schedule table** — a rolling 14-day window (operational date → +13 days), grouped with the schedule content at the top of the digest (complements the `week-strip` cards + the full page at `/production/schedule`). Columns: Date · Day · Setup · Shifts · Proj t · Act t · Status · Source. Today's row is accent-tinted, rest days dashed/muted; Status chip reuses `STATE_CHIP`/`STATE_LABEL`, Source chip is violet **Joseph** when `source` starts with `joseph:` else muted **Sheet** (matching the schedule page). Renders `null` when empty. No ₱ → no gating. |
