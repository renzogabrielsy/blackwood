...
| `schedule-month-view.tsx` | **Server (async)** | — (queries directly) | The full **month plan-vs-actual** schedule table — the ex-`/production/schedule` page body, moved into the digest world (BUG-003: it wrongly inherited the production tab shell). Rendered by `/` under `?view=schedule`; `/production/schedule` now `redirect()`s here. **Domain layer, not the digest adapter** — it queries `production_schedule` + `view_digest_prod_actual_tons` + `view_digest_daily_hours` DIRECTLY (aggregation still in SQL; the month-total footer is a display sum). Columns: Date · Day · Setup · Grades · Shifts · Proj t · Act t · Act hrs · Var · Status · Source · Remarks. Frozen `.frozen-row` header + `.frozen-row-bottom` month-total footer (solid `bg-muted`). Renders NO page container — the host owns the shell. Props: `month` (`?month=YYYY-MM`, validated; defaults to the operational month), `basePath` + `extraParams` → the prev/next month hrefs, so the host decides which sibling params survive (`/` passes `{ view: 'schedule' }`). Phone: `ScheduleCardsMobile`; `sm`+: the dense `min-w-[1080px]` table. No ₱ → no gating. **Phase-B hook (2026-07-30):** `production_schedule` now carries `owner` / `row_version` / `pending_upstream` (see `app/(app)/CONTEXT.md` → "Production-schedule ownership"). This component's read is UNCHANGED, but the editable version should select from `view_production_schedule_state` (adds `owner`, `effective_owner`, `is_reported`, `row_version`) and save via `fn_save_schedule_day` with the loaded `row_version`. Days with a parked upstream are listed by `view_production_schedule_conflicts`. |
