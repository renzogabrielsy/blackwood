...
// ---------------------------------------------------------------------
// A zero day is NOT a plunge to the floor: a day with no delivery, or one whose
// report hasn't landed yet, is kept NULL so the line never dives to zero. But
// the lines CONNECT ACROSS those nulls (connectNulls={true}) — the trend reads
// as one smooth, continuous stroke instead of a broken series of gaps.
//
// REMOVED 2026-08-28: the faint "planned rest" band and the amber "awaiting
// report" marker. Both were driven by the operational week's `production_schedule`
// plan, which is retired. Nothing in the remaining data distinguishes a planned
// rest day from an unfiled report, so the chart no longer claims to — the null
// gap and its bridging stroke, which never depended on the plan, are unchanged.

interface FlowChartRow {
  date: string;
  /** kg received; null when there was no delivery (procurement gap) */
  inKg: number | null;
  /** kg fed; null when nothing was fed or the report is still awaited */
  outKg: number | null;
}
...
function FlowChart({ flow }: { flow: FlowPoint[] }) {
  ...
  const { rows, yMax } = React.useMemo(() => {
    ...
    const built: FlowChartRow[] = flow.map((f) => ({
      ...
      // nothing fed / report pending → gap, never a plunge to zero
      outKg: f.out > 0 ? f.out : null,
    }));
      ...
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={rows} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
          ...
          <RTooltip
            ...
            labelFormatter={(l) => l}
          />
          {/* values stay null on no-report/no-delivery days (never a
