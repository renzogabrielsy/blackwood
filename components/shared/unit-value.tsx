...
//
// **PLATFORM COMPONENT (moved here 2026-09-15).** It was born in
// `app/(app)/analytics/unit-value.tsx` and lived there while it had one consumer;
// `/operations`' EOQ strip is the second, so it moved to `components/shared/` —
// CLAUDE.md's layer rule: it carries ZERO tenant knowledge (no charcoal, no
// campaign, no Supabase, not even a unit vocabulary — the glyph is a prop), so a
// second tenant's screen may use it unchanged. The analytics path is now a
// one-line re-export, so not one of its six call sites moved.
//
