...
// ─────────────────────────────────────────────────────────────────────────────
// **PLATFORM COMPONENT (moved here 2026-09-16)** from
// `app/(app)/analytics/group-print.tsx`, alongside `printCard`. `/operations`'
// price modals print one sheet per campaign through the identical machinery, so
// keeping a second copy was the drift this move exists to prevent. Zero tenant
// knowledge: the stage takes a title, a subtitle, a count label and CHILDREN.
//
// The recharts reasoning below is the measurement that decided the offstage
// shape; it is not a dependency. A stage whose children are plain tables (the
// `/operations` case) simply lays out faster than the settle window allows for.
// ─────────────────────────────────────────────────────────────────────────────
...
export interface GroupPrintStageProps {
  ...
  /**
   * The paper header — the report's name, its subtitle and the count line.
   *
   * DEFAULT TRUE, which is what /analytics has always printed. `/operations` passes
   * FALSE because each of its pages already carries its own campaign title and span
   * on the sheet, and Renzo's constraint on that report is literally *"make sure the
   * printed summary is NOT WORDY"* — a second heading restating the same two facts
   * is exactly the kind of line he was asking to remove.
   */
  showHeader?: boolean;
  ...
  return (
    ...
    <div aria-hidden className="bw-print-stage">
      <div ref={sheetRef} className="bw-print-sheet flex flex-col">
        ...
        <div className={showHeader ? "hidden print:block print:pb-3" : "hidden"}>
