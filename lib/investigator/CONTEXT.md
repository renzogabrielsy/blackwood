...
| `playbook.ts` | **P3 prompt layer (PURE).** `buildInvestigatorSystem()` = the diagnostic playbook (identity + read-only boundary + per-kind recipes + method rules + the jargon ban copied from `adjudication.ts`'s `ADJUDICATOR_SYSTEM` + the `submit_verdict` contract). `buildCaseBriefing(caseRow)` = the first user turn (report/kind/key/reason, the structured row incl. `drift_dates`, `occurrence_count`, known-ruling note). `buildChatAddendum()` (P4) now also carries a **RESOLVING** section teaching when to call `propose_resolution` (P5) — dismiss / apply / edit_apply, always "waiting for the reviewer to confirm," never "done." `BANNED_JARGON` exported for the register check. The SUSPECTED-DUPLICATE recipe carries the **L-034 month-boundary heuristic**: on the last/first day of a month, a row differing from a saved copy ONLY by the month name (e.g. "JUNE" vs "JULY", same day/weight/block/batch) is almost certainly already saved — no action needed (a kiln run crossing the boundary gets a next-month day-sheet header). **v1.1 Run Triage:** `buildTriageChatAddendum()` (the run-chat system suffix — "you are discussing a whole run; propose_group_resolution is dismiss-only; never include a flag the reviewer doubts") + `buildTriageBriefing(triage, siblings)` (the run-chat opening turn: run label + summary + one line per sibling flag with id/status/verdict). Both PURE; `case-chat.ts` swaps them in for a `run_triage` case. |
| `resolution.ts` | **P5 propose_resolution (chat-mode ONLY, client-safe).** `PROPOSE_RESOLUTION_TOOL` + `executePropose(input, caseCtx)` — a WRITE-FREE tool that validates the proposal against the case (`parseProposal` + `checkEligibility`) and echoes it back; the persisted `tool_calls` jsonb IS the proposal (no schema change). Eligibility (enforced here AND in `resolve.ts`): dismiss on any unresolved case; apply/edit_apply ONLY per-row holds with a registered writer, NOT `gate_failure`; edit_apply needs `edited_row`. Also the PURE `findOpenProposal(rows, status)` open-proposal detector (latest un-actioned `propose_resolution` with no later ruling/decline) — shared by the UI + `resolve.ts`. Wired into the chat loop in `app/(app)/sync/case-chat.ts` (NOT `runInvestigation`). The actual write is `app/(app)/sync/resolve.ts`. **v1.1 (Run Triage) GROUP resolve, also here:** `PROPOSE_GROUP_RESOLUTION_TOOL` (dismiss-ONLY) + `parseGroupProposal` + `checkGroupEligibility` (tool usable ONLY from a `run_triage` case; every id must be in the run family = the triage case's `row.case_ids`, unresolved, not the triage id itself) + `executeProposeGroup` (WRITE-FREE echo) + the PURE `findOpenGroupProposal(rows, status)` (group-tool mirror of `findOpenProposal`). The group write is `app/(app)/sync/resolve.ts::executeGroupResolution`. |
| `triage.ts` | **v1.1 Run Triage synthesis (T1).** `runTriage(runId)` → one Anthropic call (forced `SUBMIT_TRIAGE_TOOL`, no read-only tools) that clusters a run's NON-triage cases by shared root cause and writes a plain-language run summary, then UPSERTs a **synthetic case row** (`kind='run_triage'`, `report_type='run'`, `fingerprint=triageFingerprint(runId)`, `row={clusters, case_ids}`, `verdict={needs-human/high/summary…}`, `status='investigated'`) + seeds a system note + an assistant message (summary + clusters as plain text). Idempotent by the `fingerprint` UNIQUE upsert; skipped for a clean run (0 non-triage cases). PURE + exported (network-free, for `verify-triage.ts`): `parseTriage(input, validIds)` (validates + REPAIRS to a full partition — drops invented ids, dedupes across clusters, appends missing ids as a singleton needs-attention cluster), `triageFingerprint` (via `canonicalHash`), `renderSiblingForSynthesis`, `buildTriageSystem`, `renderTriageMessage`. Consts `TRIAGE_KIND='run_triage'`, `TRIAGE_REPORT_TYPE='run'`. Imports `BANNED_JARGON` from `playbook.ts`. No migration — `sync_held_cases.kind` is unconstrained TEXT, `status='investigated'` is CHECK-allowed. |
| `CONTEXT.md` | This file. |

Verify scripts: `scripts/verify-investigator-tools.ts` (31 checks, P2) + `scripts/verify-investigator-loop.ts` (34 checks, P3 — playbook recipes, jargon absence, briefing rendering, `parseVerdict` accept/reject, synthesized verdict, position math + row builders) + `scripts/verify-triage.ts` (25 checks, v1.1 — `parseTriage` validation+repair, `triageFingerprint` stability, `checkGroupEligibility` rules, `findOpenGroupProposal`, `buildTriageBriefing` rendering). All `npx tsx`, no network. The LIVE trust harness is `scripts/eval-investigator.ts` (P6) — see below. A throwaway `scripts/smoke-triage.ts` (deleted after use) proved `runTriage` live: seeded a fake run + 3 tiny cases (2 shared root cause, 1 distinct) → the model produced a clean 2+1 partition + jargon-free summary; a simulated group dismiss resolved both clustered cases with individual ledger rulings, left the distinct one untouched, cleaned to 0 orphans.
