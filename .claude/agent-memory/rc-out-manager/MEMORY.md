...
- [rc_out block_loc vs batch location_ref](feedback_rc_out_blockloc_vs_location_ref.md) — operator block_loc label can differ from the batch's physical location_ref (C/D typo) while batch identity is certain (strt_bal == delivered total) — write the operator label verbatim + flag, do NOT hold (rc_out.block_loc is free-text, not the index)
