...
- **dev → main promotion: LOCAL merge commit, no PR** (verified 2026-07-16). Precedent `d323257` "chore: merge dev into main for Vercel production launch" and my own `bacbe12` "chore: merge dev into main (Vercel build fix)" were both done via local `git checkout main && git merge --no-ff dev -m "..."` then `git push origin main` — not a GitHub PR. `main` has diverged history from `dev` (main carries its own merge commits dev doesn't have), so `git merge --ff-only` always fails with "Diverging branches" — go straight to `--no-ff`. Trigger for this promotion: explicit request to sync a git-connected Vercel *production* deployment target with dev (a build/config fix that must reach prod), not a routine feature landing.
