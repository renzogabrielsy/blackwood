...
## "Closed Blocks" summary data layer — view_rc_out_closed_blocks (2026-06-29) — [[rc-out-closed-blocks]]

Read-only SECURITY-INVOKER view (migration `20260629000000_create_view_rc_out_closed_blocks.sql`, MCP apply + .sql on disk, `GRANT SELECT authenticated,anon`) = ONE summary row per CLOSED block, all summing in SQL. Base = `rc_out JOIN batches WHERE status='CLOSED'` → 440 rows (NOT 449: 9 closed batches have zero rc_out = test/QA junk, dropped; every row close_date non-null). Cols (FE contract, types regenerated): batch_id, batch_code, block_loc (`COALESCE(NULLIF(trim(location_ref),''), latest non-empty rc_out.block_loc)` — NULL ok for FEED), close_date (`COALESCE(MAX(txn) FILTER CLOSED-remark, MAX(txn))`), total_fed_kg, feed_count, first_fed_date, total_value, avg_price. PRICE = deliveries.cost_basis weighted-avg per batch (NOT rc_out — no price col exists; NOT batches.avg_cost — stale per [[rc-movement-fed-price]]); total_value=fed*unitcost, avg_price=blended ₱/kg, NULL when no priced deliveries. NOT gated in view — FE nulls total_value/avg_price downstream when `!canViewPrices()` ([[price-gating]]), like view_blocking_grid. Verified live (JULY-25-BLK5 58546kg/C-11A/₱40.91, JUNE-26-FEED5 25990kg/null-block, FEB-25-BLK8 no-marker→2025-04-30). rc-out CONTEXT.md updated. See rc-out-closed-blocks.md.
