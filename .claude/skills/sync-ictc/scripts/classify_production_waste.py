#!/usr/bin/env python3
"""
Classify extracted PRODUCTION WASTE rows against existing production_waste rows.
...
Parent-child shift model (PRODUCTION_DESIGN.md §3, §6):
  ...
  production_waste  is a CHILD     — natural key (shift_id); UNIQUE(shift_id) => exactly 1 per shift

Because production_waste no longer stores date/batch/shift, this classifier first resolves each
extracted row's (transaction_date, production_batch, shift) triplet to an existing shift_id via
--shifts-json. The row's natural key is then (shift_id,) alone.

Outcomes per extracted waste row (mirrors classify_rc_out.py vocabulary):
  - NEW              : no waste row for that shift_id -> queue for INSERT.
                       May carry needs_shift_upsert=true when the parent shift does not yet exist.
  - VALUE_CHANGED    : a waste row exists for that shift_id, >=1 comparable field differs
                       -> include diff + existing_id.
  - DUPLICATE_NOOP   : a waste row exists for that shift_id, all comparable fields equal
                       (numeric tolerance 0.01) -> skip.
  - MALFORMED        : missing/invalid required field -> NEVER written. For waste:
                         * shift null/empty (cannot key to production_shifts)
                         * any waste stream negative

Natural key = shift_id only. The compared fields are the 8 streams + remarks:
  rs1a_kg, rs1b_kg, bf_kg, rs23_kg, rs5_kg, trml1_kg, trml2_kg, grit_kg, remarks.
(`ttl_waste_kg_reported` / `_summed_kg` are extractor-side reconciliation aids, NOT stored — excluded.)
...
Usage:
    python3 classify_production_waste.py \\
        --extract-json /tmp/.../out_ivy.json \\
        --db-rows-json /tmp/.../production_waste_rows.json \\
        --shifts-json  /tmp/.../production_shifts.json \\
        --output       /tmp/.../classified_production_waste.json \\
        [--verbose]

--extract-json : the Ivy extractor output; its "waste" array is read.
--db-rows-json : array of existing production_waste child rows in the date window, DENORMALIZED —
...
# ---------------------------------------------------------------------------
def main() -> int:
    parser = argparse.ArgumentParser(description="Classify extracted waste against production_waste.")
    parser.add_argument("--extract-json", required=True, help="Ivy extractor output (reads 'waste').")
    parser.add_argument("--db-rows-json", required=True,
                        help="Denormalized production_waste rows (child JOINed to parent shift).")
    ...
    extracted_rows = extracted_data.get("waste", []) if isinstance(extracted_data, dict) else extracted_data
    ...
    for idx, ex in enumerate(extracted_rows):
        ...
        if needs_shift_upsert:
            ...
            classifications.append({
                ...
                "record": ex, "reasons": ["parent shift absent; upsert shift then insert waste"],
                "confidence": 0.95,
            })
            counts["new"] += 1
            continue
    ...
    if args.verbose:
        print("=== production_waste Classification Summary ===", file=sys.stderr)
