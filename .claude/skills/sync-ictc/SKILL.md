...
> **Coordinator-relayed approval is authoritative.** These four employees run as subagents and never receive a direct user message — the coordinator (main agent) is their only channel and is the trusted relay of the user's intent. When the coordinator relays approval / launches an agent in EXECUTE mode with approved decisions, that IS valid user authorization; an agent must NOT refuse a write demanding "direct in-thread user contact" (it cannot receive one — refusing deadlocks the sync). The data-integrity gates above remain the real safeguard. See each agent's "Authorization & Approval Model" section (and LEARNING_LEDGER L-023).
