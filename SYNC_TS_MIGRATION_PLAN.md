...
- [x] **Follow-up DONE:** Mail Clerk no longer pulls full message sources. `gmail.searchLatestAttachment` does a `bodyStructure`-first metadata pass (`findAttachmentPart`) then downloads ONLY the newest xlsx **part** (`download(uid, part)`), with a per-report fallback to the old full-source path. Fetch time dropped from ~2m40s to ~65–110s; bytes/filenames byte-identical. Added a 60s `connectionTimeout` so a connect stall errors fast instead of hanging to the 5-min socket timeout.
- [x] **Live progress DURING the fetch DONE:** the fetch now emits per-report beats (pct 4→25) — "Connecting to Gmail…" → per report "Looking for … / Found … (size) / Downloaded N of 7 reports…", real filenames+sizes, ≤2 beats/report — so the "Run Sync" panel moves report-by-report instead of freezing on "Checking Gmail…". `mailClerkBody` wires the real `_run` emitter; `runMailClerk`/the live-test pass an `onProgress` logger. Unit-tested (`test/workflows/mailClerk.test.ts`, `findAttachmentPart` in `test/gmail.test.ts`).
