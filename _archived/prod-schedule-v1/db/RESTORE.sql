-- Production-schedule feature — DATABASE ARCHIVE / RESTORE
--
-- WHAT THIS IS
--   Renzo is removing the in-app production-schedule feature (he maintains the
--   schedule in Google Sheets; a future v2 will be a read-only gsheet view). This
--   file is the complete, restorable DDL of every database object the removal
--   touches. NOTHING here is unrecoverable.
--
-- WHERE THE DATA IS
--   graveyard.production_schedule_20260827   — all 273 rows, structural copy of the
--                                              table (plan_date 2026-01-01 .. 2026-09-30;
--                                              35 rows owner='human', 24 with a
--                                              non-null pending_upstream).
--   graveyard.prod_schedule_ddl_20260827     — this same DDL, stored IN the database,
--                                              one row per object.
--
--   The graveyard schema has NO grants to anon / authenticated / service_role and
--   RLS is on with no policies. Only the table owner (postgres) can read it. It is
--   a vault, not a feature.
--
-- HOW TO RESTORE (full rollback of the removal)
--   1. Run the "public.production_schedule" block below — it recreates the table,
--      indexes, constraints, RLS policies, grants and comments (empty).
--   2. Copy the rows back:
--          INSERT INTO public.production_schedule
--          SELECT * FROM graveyard.production_schedule_20260827;
--      (Column order is identical — the archive was made with LIKE.)
--   3. Run the two view blocks and the three function blocks, in the order they
--      appear below.
--   4. Run the "view_digest_stream_status" PRE-REWRITE block to put the
--      schedule-based missed_working_days arithmetic back. It is CREATE OR REPLACE,
--      so the grants the sync worker depends on are preserved.
--   5. Re-verify the worker can still read everything:
--          npx tsx scripts/verify-worker-view-grants.ts
--   6. Regenerate types:
--          supabase gen types typescript --linked > types/supabase.ts
--
-- ORDER MATTERS. The views and functions all reference public.production_schedule,
-- so the table block must run first or every one of them fails.
--
-- TWO THINGS NOT TO DO
--   * Do not restore view_digest_stream_status with DROP VIEW + CREATE VIEW. DROP
--     loses its grants, and the service_role grant is what keeps the sync worker's
--     stream-freshness watch from going silently blind (L-044).
--   * Do not make anything here SECURITY DEFINER to shortcut a permission error.
--     Grant instead.
--
-- Archived 2026-08-28 from the live database (catalog-verified: pg_get_viewdef /
-- pg_get_functiondef / pg_constraint / pg_index / pg_policy / role_table_grants).
-- Deleting the archive itself is a separate, explicit decision — not part of the
-- removal.

-- >>> TABLE: public.production_schedule
...
-- because it was made with CREATE TABLE ... (LIKE public.production_schedule ...).


-- >>> VIEW: public.view_production_schedule_state
...
-- >>> VIEW: public.view_production_schedule_conflicts
...
-- >>> FUNCTION: public.fn_apply_schedule_upstream(jsonb)
...
-- >>> FUNCTION: public.fn_save_schedule_day(date, integer, jsonb, boolean)
...
-- >>> FUNCTION: public.fn_release_schedule_day(date, integer)
...
-- >>> VIEW: public.view_digest_stream_status
...
-- >>> VIEW: public.view_digest_stream_freshness
