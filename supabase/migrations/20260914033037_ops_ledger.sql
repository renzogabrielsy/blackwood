...
-- -----------------------------------------------------------------------------
-- 11. fn_ops_ledger_verify — the verification probe behind scripts/verify-ops-ledger.ts
-- -----------------------------------------------------------------------------
-- Every view above is authenticated-only by design, so neither the anon key nor
-- the service-role key can read one over PostgREST — and a verification script
-- has no other way in. That is the whole reason this exists, and it is the same
-- reason `canonical_supplier_batch_probe` and `fn_audit_trigger_function_grants`
-- exist. SECURITY DEFINER, STABLE, `service_role` EXECUTE only.
--
-- IT RETURNS NO ₱ VALUE. Every money-derived check is published as a GAP that
-- must be exactly 0; a zero difference says nothing about the numbers underneath
-- it. Zero on every `*_mismatch` key is the passing state.
create or replace function public.fn_ops_ledger_verify()
returns jsonb
language sql
stable
security definer
...
with
 counts as (
   select
     (select count(*) from public.view_ops_ledger_day)                 as day_rows,
     (select count(distinct campaign_key) from public.view_ops_ledger_day) as day_campaigns,
     (select count(*) from public.view_ops_ledger_day where is_rest_day)   as rest_days,
     (select sum(ledger_days) from public.view_ops_ledger_campaign_kpis)   as ledger_days_total,
     (select max(span_days) from public.view_ops_ledger_campaign_span)     as max_span_days,
     (select count(*) from public.view_ops_ledger_campaign_span)           as span_rows,
     (select count(*) from public.view_ops_ledger_campaign_kpis)           as kpi_rows,
     (select count(*) from public.view_ops_ledger_shift)                   as shift_rows,
     (select count(*) from public.view_ops_ledger_day_grade)               as day_grade_rows,
     (select count(*) from public.view_ops_ledger_day_block)               as day_block_rows,
     (select count(*) from public.view_ops_ledger_day_blocks_used)         as blocks_used_rows,
     (select count(*) from public.view_ops_ledger_campaign_grades)         as campaign_grade_rows,
     (select max(c) from (select count(*) c from public.view_ops_ledger_day_block group by campaign_key) t) as max_day_block_rows_per_campaign,
     (select max(c) from (select count(*) c from public.view_ops_ledger_day_grade group by campaign_key) t) as max_day_grade_rows_per_campaign,
     (select max(c) from (select count(*) c from public.view_ops_ledger_day group by campaign_key) t)       as max_day_rows_per_campaign
 ),
 folds as (
   select
     (select count(*) from public.view_ops_ledger_day d
        left join (select campaign_key, calendar_date, sum(kg) kg from public.view_ops_ledger_day_grade group by 1,2) g
          on g.campaign_key=d.campaign_key and g.calendar_date=d.calendar_date
       where d.produced_kg is distinct from g.kg) as grade_fold_mismatch,
     (select count(*) from public.view_ops_ledger_day d
        left join (select campaign_key, calendar_date, sum(fed_kg) kg from public.view_ops_ledger_day_block group by 1,2) b
          on b.campaign_key=d.campaign_key and b.calendar_date=d.calendar_date
       where d.fed_kg is distinct from b.kg) as block_fold_mismatch,
     (select count(*) from public.view_ops_ledger_day d
        left join (select campaign_key, calendar_date, sum(produced_kg) p from public.view_ops_ledger_shift group by 1,2) s
          on s.campaign_key=d.campaign_key and s.calendar_date=d.calendar_date
       where d.produced_kg is distinct from s.p) as shift_produced_mismatch,
     (select count(*) from public.view_ops_ledger_day d
        left join (select campaign_key, calendar_date, sum(downtime_hours) dt from public.view_ops_ledger_shift group by 1,2) s
          on s.campaign_key=d.campaign_key and s.calendar_date=d.calendar_date
       where d.downtime_hours is distinct from s.dt) as shift_downtime_mismatch,
     (select count(*) from (select campaign_key, sum(fed_kg) f, sum(produced_kg) p
                              from public.view_ops_ledger_day group by 1) t
        join public.view_ops_ledger_campaign_kpis k on k.campaign_key=t.campaign_key
       where t.f is distinct from k.fed_kg or t.p is distinct from k.produced_kg) as day_totals_vs_kpi_mismatch,
     (select count(*) from (select campaign_key, grade, sum(kg) kg from public.view_ops_ledger_day_grade group by 1,2) t
        full join public.view_ops_ledger_campaign_grades g on g.campaign_key=t.campaign_key and g.grade=t.grade
       where t.kg is distinct from g.kg) as campaign_grade_fold_mismatch,
     (select count(*) from public.view_ops_ledger_campaign_kpis where ledger_days is distinct from span_days) as ledger_days_vs_span_mismatch
 ),
 reuse as (
   select
     (select count(*) from public.view_ops_ledger_campaign_kpis k
        join public.view_analytics_batch_cost bc on bc.production_batch=k.production_batch and bc.campaign_year=k.campaign_year
       where k.fed_kg is distinct from bc.fed_kg
          ...
          or k.blocks_fed is distinct from bc.blocks_fed or k.blocks_closed is distinct from bc.blocks_closed
          or k.blocks_open is distinct from bc.blocks_open or k.blocks_in_price is distinct from bc.blocks_in_price
          ...
          or k.sundry_kg is distinct from bc.sundry_kg or k.out_kg is distinct from bc.out_kg) as kpi_vs_batch_cost_mismatch,
     (select count(*) from public.view_ops_ledger_campaign_kpis k
        join public.view_analytics_production_by_batch p on p.production_batch=k.production_batch and p.campaign_year=k.campaign_year
       where k.produced_kg is distinct from p.produced_kg
          or k.production_reported is distinct from coalesce(p.production_reported,false)
          or k.reported_days is distinct from p.reported_days
          or k.shift_count is distinct from p.shift_count
          or k.run_count is distinct from p.run_count
          or k.downtime_hours is distinct from p.downtime_hrs
          or k.sacks is distinct from p.sacks
          or k.kwh is distinct from p.kwh
          or k.kwh_per_produced_kg is distinct from p.kwh_per_produced_kg) as kpi_vs_production_by_batch_mismatch,
     (select count(*) from public.view_ops_ledger_campaign_kpis k
        left join public.view_analytics_batch_cost bc on bc.production_batch=k.production_batch and bc.campaign_year=k.campaign_year
       where bc.production_batch is null) as kpi_without_batch_cost_row
 ),
 q3d as (select sum(fed_kg) f, sum(produced_kg) p, sum(fed_value_php) v
           from public.view_ops_ledger_campaign_kpis
          where campaign_key in ('JULY-2026','AUGUST-2026','SEPTEMBER-2026')),
 q3g as (select * from public.fn_ops_ledger_group_kpis(array['JULY-2026','AUGUST-2026','SEPTEMBER-2026'])),
 q3 as (
   select
     q3g.campaign_count                                   as q3_campaign_count,
     coalesce(q3g.yield_pct - (q3d.p / nullif(q3d.f,0)), 0)                     as q3_gap_yield,
     coalesce(q3g.fed_php_kg - (q3d.v / nullif(q3d.f,0)), 0)                    as q3_gap_fed_rate,
     coalesce(q3g.php_per_produced_kg_delivered - (q3d.v / nullif(q3d.p,0)), 0) as q3_gap_pc_rate,
     coalesce(q3g.fed_kg - q3d.f, 0)                      as q3_gap_fed_kg,
     coalesce(q3g.produced_kg - q3d.p, 0)                 as q3_gap_produced_kg,
     q3g.blocks_fed_distinct                              as q3_blocks_distinct,
     q3g.blocks_fed_campaign_sum                          as q3_blocks_campaign_sum,
     q3g.campaigns_fully_covered                          as q3_campaigns_fully_covered,
     (q3g.php_per_produced_kg_true is null)               as q3_true_pc_cost_is_null,
     (q3g.php_per_produced_kg_true_covered is not null)   as q3_true_pc_cost_covered_present,
     q3g.ledger_days                                      as q3_ledger_days,
     q3g.active_days                                      as q3_active_days,
     q3g.rest_days                                        as q3_rest_days,
     q3g.reported_campaign_days - q3g.reported_calendar_days as q3_changeover_surplus
   from q3g, q3d
 ),
 miss as (
   select (select coalesce(array_length(campaigns_missing,1),0) from public.fn_ops_ledger_group_kpis(array['NOPE-1999'])) as unknown_key_reported
 ),
 posture as (
   select
     (select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
       where n.nspname='public' and c.relname like 'view_ops_ledger%'
         and coalesce((select option_value from pg_options_to_table(c.reloptions) where option_name='security_invoker'),'false') <> 'true') as not_security_invoker,
     (select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
       where n.nspname='public' and c.relname like 'view_ops_ledger%' and not has_table_privilege('authenticated', c.oid, 'SELECT')) as missing_authenticated_select,
     (select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
       where n.nspname='public' and c.relname like 'view_ops_ledger%' and has_table_privilege('anon', c.oid, 'SELECT')) as anon_can_select,
     (select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
       where n.nspname='public' and c.relname like 'view_ops_ledger%' and has_table_privilege('service_role', c.oid, 'SELECT')) as service_role_can_select,
     (select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
       where n.nspname='public' and c.relname like 'view_ops_ledger%') as view_count,
     (select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
       left join pg_description d on d.objoid=c.oid and d.objsubid=0
       where n.nspname='public' and c.relname like 'view_ops_ledger%' and d.description is null) as views_without_comment,
   ...
   select
     (select count(*) from information_schema.columns
       where table_schema='public'
         ...
         and column_name ~* 'php|peso|cost|price|value|amount') as money_named_in_peso_free_views,
     (select count(*) from information_schema.columns
       where table_schema='public' and table_name='view_ops_ledger_day'
         and column_name ~* 'php|peso|cost|price|value|amount') as money_named_in_day_view
 )
select to_jsonb(counts) || to_jsonb(folds) || to_jsonb(reuse)
       || to_jsonb(q3) || to_jsonb(miss) || to_jsonb(posture) || to_jsonb(money)
from counts, folds, reuse, q3, miss, posture, money;
$$;

comment on function public.fn_ops_ledger_verify() is
'READ-ONLY verification probe for the ops-ledger data layer, the instrument behind scripts/verify-ops-ledger.ts. It exists because every view_ops_ledger_* view is authenticated-only by design, so neither the anon key nor the service-role key can read them over PostgREST and a verification script has no other way in — the same reason canonical_supplier_batch_probe and fn_audit_trigger_function_grants exist. SECURITY DEFINER, STABLE, service_role EXECUTE only. IT RETURNS NO ₱ VALUE: every money-derived figure is published as a GAP that must be exactly 0, and a zero difference says nothing about the numbers underneath it. Zero on every *_mismatch key is the passing state.';

revoke execute on function public.fn_ops_ledger_verify() from public, anon, authenticated;
grant execute on function public.fn_ops_ledger_verify() to service_role;


-- -----------------------------------------------------------------------------
-- 12. fn_ops_ledger_verify_groups — the 32-campaign group proof, on its own
-- -----------------------------------------------------------------------------
-- Split out of `fn_ops_ledger_verify` for one measured reason: it calls
-- `fn_ops_ledger_group_kpis` once per campaign — 32 times, each call re-walking
-- the whole day spine and the whole block lens — and together with the folds it
-- ran past PostgREST's gateway timeout. Same posture as its sibling: SECURITY
-- DEFINER, STABLE, `service_role` EXECUTE only, and NO ₱ VALUE in the result.
create or replace function public.fn_ops_ledger_verify_groups()
returns jsonb
language sql
stable
security definer
...
with
 single as (
   select count(*) as single_campaign_group_mismatch
     from public.view_ops_ledger_campaign_kpis k
     cross join lateral public.fn_ops_ledger_group_kpis(array[k.campaign_key]) g
...
select to_jsonb(single) from single;
$$;

comment on function public.fn_ops_ledger_verify_groups() is
'READ-ONLY: proves a ONE-campaign group returned by fn_ops_ledger_group_kpis equals that campaign''s own view_ops_ledger_campaign_kpis row, column for column, on EVERY campaign. Split out of fn_ops_ledger_verify because 32 group calls plus the fold checks exceeded PostgREST''s gateway timeout. SECURITY DEFINER, service_role EXECUTE only; returns a single count, never a ₱ value. single_campaign_group_mismatch = 0 is the passing state.';

revoke execute on function public.fn_ops_ledger_verify_groups() from public, anon, authenticated;
grant execute on function public.fn_ops_ledger_verify_groups() to service_role;
