-- Monthly analytics rollup of the deliveries table for the "Analyst Brief"
-- (price/volume) feature. All aggregation lives HERE per the project rule:
-- never compute weighted averages or balances in TypeScript.
--
-- Grain: one row per (year, month) that has at least one delivery.
...
-- ---------------------------------------------------------------------------
-- Yearly footer rollup companion. Same weighting rules at the YEAR grain so
-- the action's totalsByYear footer is computed in SQL (NOT by averaging the
-- monthly averages, which would be wrong for a weighted metric).
-- ---------------------------------------------------------------------------
