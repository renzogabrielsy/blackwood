...
- `fn_recompute_batch_state(p_batch_code text) → void` — the ONE definition of a batch's derived state. Recomputes `batches.current_weight` (deliveries − rc_out) and `batches.avg_cost` (delivery-weighted, **over PRICED deliveries only** — `cost_basis > 0`, narrowed 2026-08-07, see Database Rules) from the base tables. Idempotent, so it doubles as the backfill. Called by every branch of `fn_update_blackwood_state`. **EXECUTE is granted to `service_role` AND `authenticated` — the latter is NOT optional (2026-08-14).** `fn_update_blackwood_state` is SECURITY INVOKER, so the trigger body runs as whoever wrote the row; `authenticated` is therefore a *calling role of this helper via the trigger*, and while it was `service_role` only **every in-app delivery INSERT/UPDATE/DELETE died with `permission denied for function fn_recompute_batch_state` (SQLSTATE 42501) and rolled back** — for nine days, because the sync writes with the service-role key and never saw it. Direct exposure to a client is safe: the function derives everything it writes from the base tables, so calling it by hand can only recompute truth. `anon` still has nothing. Migration `20260814025344`; guarded by `fn_audit_trigger_function_grants` / `scripts/verify-trigger-grants.ts`.
- `fn_audit_trigger_function_grants(p_role name DEFAULT 'authenticated') → TABLE(...)` — the **regression guard** for that class of bug (2026-08-14). For every non-internal trigger on a table `p_role` can write, it walks the SECURITY INVOKER call graph out from the trigger function and returns any reachable function `p_role` cannot EXECUTE. **Zero rows is the passing state.** SECURITY DEFINER callees terminate a branch (they re-root privileges to their owner). Edges are found by word-boundary matching callee names against `prosrc` — Postgres records no function→function dependency — so it can over-report but never under-report a plain call; if it ever fires on something benign, DEMOTE it with a written reason rather than widening the predicate. Read-only, STABLE, `service_role` only. Run it via `npx tsx scripts/verify-trigger-grants.ts` after any migration that adds a trigger, a trigger function, or a GRANT/REVOKE.
...
- **A SECURITY INVOKER trigger function runs as the WRITER, so every function it calls must be executable by every role that can write the table (2026-08-14, L-043).** The project rule is "grant back only the roles that call it" — and **a role that fires a trigger IS a calling role of everything that trigger reaches.** `fn_recompute_batch_state` was granted to `service_role` only, so from 2026-08-04 to 2026-08-14 every in-app delivery write failed with `permission denied for function fn_recompute_batch_state` while the service-role sync stayed green: **a privileged writer masking a completely broken unprivileged path, with a human error toast as the only symptom.** When adding or granting a function, ask which roles reach it *through a trigger*, not just which call it by name. **Do NOT fix this shape by making the trigger function SECURITY DEFINER** — that re-roots the privilege context of everything the trigger does, including its table writes, and bypasses RLS on the main write path; grant the helper instead. Enforced by `scripts/verify-trigger-grants.ts`.
- **A view the sync worker reads must be granted to `service_role`, and the grant must cover the WHOLE `security_invoker` dependency chain (2026-08-18, L-044).** The mirror image of L-043 above, roles reversed: there a privileged writer masked a broken unprivileged path; here a privileged *reader* (the app, as `authenticated`) masked a broken **service-role** path. The worker reads exactly five views; two of them — `view_digest_stream_status` and `view_digest_unpriced_deliveries` — returned **HTTP 403 / SQLSTATE 42501** to the service-role key, and they are **precisely the two that carry an alarm**: the stream-freshness watch (2026-08-04) and the unpriced-delivery chase (2026-08-07, L-039 rule 5). Both reads sat behind a bare `catch`, so a permission error was rendered as *"nothing to report"* from the day each alarm was built — `sync_runs.result.reconciliation.stale_streams` was **absent on every run in the table**, i.e. the freshness watch had never once fired, including on days the view plainly reported a stream late. **The trap is that a correct-looking grant on the outermost view is not enough.** `view_digest_unpriced_deliveries` HAD its `service_role` grant (`20260807040107` granted it `TO authenticated, service_role`, correctly anticipating the worker) and still 403'd: reporting views are `security_invoker`, so the CALLER's privileges are applied to every relation underneath and **the denial cascades, naming the DEPENDENCY rather than the view you asked for** — `permission denied for view view_digest_operational_days`. The unit of correctness is the **closure**, walked with `pg_depend`/`pg_rewrite`, never eyeballed. Note the direction of the arrow matters: `view_digest_stream_freshness` and `view_digest_unpriced_recent` are **consumers** of the worker's views, not dependencies of them, so they stay `authenticated`-only. `service_role` holds `rolbypassrls`, so RLS is never the obstacle here — the obstacle is a plain table-level privilege and the instrument is a plain **GRANT**. **Do NOT fix this shape by re-rooting a view to `security_definer`** — that makes it run as its owner for *every* caller, a permanent privilege change to paper over a missing grant. And remember `DROP VIEW` + `CREATE VIEW` **loses** its grants (`CREATE OR REPLACE VIEW` keeps them). Migration `20260818071855`; enforced by `scripts/verify-worker-view-grants.ts`, which derives the view list from the worker source (never a hand-kept list) and proves readability by **actually reading as `service_role`** rather than inspecting the grant table — the L-043 lesson that a permission fix is proven by assuming the victim's role. Zero findings is the passing state; an empty extraction is a **failure**, not a vacuous pass.
...
**A total that equals the sum of the parts is not a second alarm — the blocking grand total reports its RESIDUAL (2026-08-12).** The Blocking cross-check's grand-total finding was hard-coded `high`, and across **all 11 stored runs** that ever produced a block diff the per-block gaps summed to the grand-total delta **exactly every time** (run `dc944b54`: 6,240 + 23,264 + 3,669 + 2,975 = 36,148 kg). It was therefore crying wolf on essentially every run while telling the operator nothing the block rows had not already said. It now also computes **`residual = grand-total delta − Σ(signed per-block gaps)`**, and the severity follows the residual, not the delta: **zero residual → `attention`**, saying explicitly that every kilogram is accounted for by the blocks listed and that this is **consistent with** the Sheet's Blocking tab not yet reflecting recent feeding — **the cause is never asserted** ("LIKELY, not definitely" — Renzo); **non-zero residual → stays `high`**, naming the kilograms that **no flagged block explains**, which is the genuinely alarming state and includes the extreme where no block was flagged at all. Two rules generalise beyond this check: **(1) a block's contribution is `(sheet_kg ?? 0) − (computed_kg ?? 0)`, never its `delta` field** — a presence diff ("the Sheet has a batch here, the app has no active batch") carries `delta: null` yet its whole balance is real gap, and summing `delta` would have fabricated a 26,239 kg residual on `dc944b54`; signs are preserved so opposite directions cancel rather than add. **(2) The "is it zero" test reuses the check's OWN tolerance (100 kg)** rather than inventing a second threshold. Presentation and severity only: no tolerance moved, no per-block finding changed, nothing that was reported before stopped being reported, and the case fingerprint is untouched. The four figures ride as structured finding data (`accounted_block_kg`, `accounted_block_count`, `residual_kg`, `fully_accounted`) because the Excel report and the panel must be able to filter and total them, not just read a sentence. Full detail: `workers/sync/src/reconcile/CONTEXT.md` § "B2's RESIDUAL", `SYNC_VALIDITY_RULESET.md` § B1–B5.
...
**An operator's SHORTHAND is a naming convention to be learned, NOT malformed input (2026-08-13, L-042).** The third instance this month of the same confusion — a *name a human typed* mistaken for *bad data* (L-039: `"Aug. 2026"` vs a generated `"August 2026"`; L-040b: `FEEDING # 1` vs `JULY-26-FEED1` inside a natural key). Two changes to the deliveries pipeline, one lesson:

1. **`FEEDING # N` is accepted.** `FEEDING_AREA_RE` matched only `FEEDING AREA <N>` — the **Sheet's** spelling. MC types `FEEDING # 1` in column D, so the label fell through to the raw-value branch: **truthy** (passed the malformed guard), **not pattern-valid** (never auto-created), therefore **held on every run forever**. Two real truckloads were stuck (2026-08-05 / AAV 6111 / 19,185 kg for a week; 2026-08-12 / KCA 378 / 18,650 kg). It now also accepts `FEEDING #2`, `FEEDING NO. 2`, `FEEDING 2`, `FEEDING AREA #2`, a trailing period and a bare `FEEDING` (numberless → the pre-existing needs-mapping warning, unchanged) — and **produces EXACTLY what `FEEDING AREA N` produced**, no new output format. Deliberately still rejected: `FEEDING AREA A`, `FEEDING AREA 1 AND 2`, `RE-FEEDING 1` (`REFEED` is its own family), and bare `FEED` (a batch is literally named that).
2. **"Not filled in yet" is no longer called MALFORMED.** MC books overnight weights in early with only plate, weight and moisture and assigns the pile later; those rows were reported as *"Row could not be read"* (two on 2026-08-12, both self-fixed by morning). They now go to a separate `awaiting_assignment` bucket → an `awaiting_batch_assignment` run finding (`section: 'deliveries'`, existing vocabulary, no parallel taxonomy). **Never held** — no durable case, no blocked watermark — and **severity escalates with age** (`info` 0–1d → `attention` 2–3d → `high` 4+d, measured in **Asia/Manila**), because the row is not in the database so nothing else in the system can ever notice it.

**Two rules generalise beyond this fix.** (1) **An alias table you already own must be CONSULTED, not just documented.** `batch_code` has always had two month-prefix conventions and `batchCodeFallbacks` has encoded them for months — but no predicate answered *"is this pair merely a spelling difference?"*, so every consumer compared RAW STRINGS. The extractor derives a FEED code from the FULL month name (`AUGUST-26-FEED1`) while the DB spells that batch **`AUG-26-FEED1`** (measured: August *feed* is `AUG-`, August *blocks* are `AUGUST-`; both conventions live simultaneously), so under the two-tier identity the row matched on tier 1 and then "disagreed" on `batch_code` — accepting the shorthand without the alias would merely have moved the stall from `unmapped` to `cross_batch_reassignment`. `workers/sync/src/lib/batchCodeAlias.ts` is now THE definition, used in **two** places that answer different questions: `batchCodeAliasEqual` in `field_differences` (do the sources actually disagree — this is what makes the 08-05 row a clean NOOP) and `resolveKnownBatchCodeAlias` in the guard (what a genuinely NEW row is WRITTEN as). It is **not a fuzzy matcher**: only the existing table's prefix pairs collapse, only when year and suffix are byte-identical — `JULY-26-BLK9` vs `JUNE-26-BLK9` (the L-033 phantom, and both deliveries parity fixtures) is untouched. (2) **Trading a loud wrong for a quiet wrong is a regression.** Skipping the guard's re-spell would have replaced an obviously-junk `FEEDING # 3` batch with a plausible-looking duplicate `AUGUST-26-FEED3` beside the real `AUG-26-FEED3`. The re-spell is safe because it borrows L-033b's property verbatim: **it can only ever point at a batch that ALREADY EXISTS, and never overrides a code that already resolves.**

**And one thing the split got right that is easy to get wrong: split by a POSITIVE signal, never by widening the bucket.** The soft class requires a **truck plate**. That single clause is what keeps MALFORMED loud, because an **orphan wet-recovery sub-row** has the same missing batch code, IS genuinely bad data, and is *defined* by having no plate, no batch and no block — so it fails the clause and stays malformed. When the two cannot be told apart, the loud answer wins. Extractor, classifier and guard moved in lockstep with the Python oracle; parity is green with no new deviation. Full spec: **`workers/sync/specs/deliveries.md` §11**; ledger **L-042**.

**KNOWN, NOT FIXED (needs an approved cleanup):** `batches` contains the raw-label batches this bug created — `FEEDING # 1`, `FEEDING # 2` (**holding 18,650 kg of phantom weight** while `AUG-26-FEED2` reads −3,000 kg) and `FEEDING AREA # 1`…`# 4` — plus the 2026-08-12 delivery stored under `batch_code = 'FEEDING # 2'`. No DB write was made by this change.
